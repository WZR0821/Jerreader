...
private fun QuickStartSection() {
    GuideSection("快速开始") {
        GuideStepRow(1, "导入书籍", "在“书架”点“导入书籍”，选择 EPUB、PDF、DOCX 或 TXT 文件；导入完成后会直接打开。")
        GuideStepRow(2, "打开阅读", "轻点封面进入阅读器；应用会自动保存每本书的阅读位置和排版偏好。")
        GuideStepRow(3, "选择翻译", "在“设置 → 翻译与 AI → 翻译范围”中选择“句子”或“段落”，再轻点正文即可一次翻译一个句子或段落；长按会启用系统选区，并可从菜单选择翻译、查词或划线。")
...
private fun ImportSection() {
    GuideSection("如何导入") {
        GuideItem(
            JerreaderIcon.FOLDER,
            "从系统文件导入",
            "书架 → “导入书籍” → 在系统文件选择器中选择文件。导入后直接进入阅读，重复文件会打开书架中的现有副本。支持设备存储、下载目录、云盘和外接存储。"
        )
        GuideItem(
...
private fun ReadingSection() {
    GuideSection("如何阅读与翻译") {
        GuideItem(
            ...
            "翻页与阅读控制",
            "左右滑动翻页，也可用底部控制栏的单箭头按钮翻页、双箭头按钮跳章；进度条可拖到整本书的任意位置。" +
                "轻点正文的空白处（没有文字的地方）可以收起或重新显示上下两条控制栏。"
        )
        GuideItem(
            JerreaderIcon.SPEECH_BUBBLE_FILLED,
            "点句或点段翻译",
            "底部控制栏中间的气泡按钮为高亮状态时表示已开启轻点翻译；可在“设置 → 翻译与 AI → 翻译范围”选择句子或段落，" +
                "再轻点正文翻译。开启“轻点翻译时禁用点按翻页”后，滑动仍可翻页，点按不会同时触发翻页。" +
                "轻点没有文字的位置不会触发翻译，只会收起或显示控制栏。"
        )
        GuideItem(
            JerreaderIcon.CHECKLIST,
            "精确框选",
        ...
        GuideItem(
            JerreaderIcon.SWAP,
            "跨页句段",
            "句子在页边界被截断时，打开译文卡片右上角的更多菜单，选“扩展跨页句段”；" +
                "应用会先在本机补齐可确认的句子，再只请求一次翻译。没有可靠上下文时不会臆造内容。"
        )
        GuideItem(
            JerreaderIcon.BOOKS,
            "PDF 翻译",
...
private fun FeatureSection() {
    GuideSection("功能介绍") {
        GuideItem(
            JerreaderIcon.PALETTE,
            "界面主题",
            "设置 → 界面主题可选择海蓝、森林、紫藤、琥珀或莓红色系；按钮、卡片和背景会一起变化，并自动适配系统浅色或深色外观。"
        )
        GuideItem(
            JerreaderIcon.TEXT_FORMAT,
            "阅读设置与选区色",
            "每本书可独立保存字体、字号、背景、页边距、行距、段距、翻页模式和选区颜色。浅色、护眼、冷灰、深色会自动采用适配的选区色；自定义背景可另选选区色，并把这一组配色存成自己的背景套系。设置 → 默认阅读排版可决定新书样式，并用开关持续同步或立即应用到现有全部书籍。"
        )
        GuideItem(
            JerreaderIcon.TABLE_OF_CONTENTS,
            "目录、搜索、书签与批注",
            "顶部控制栏的目录按钮集中提供章节目录、全文搜索、书签和划线批注；点正文上的高亮可直接编辑该条批注。" +
                "右侧的“格式”按钮打开这本书自己的阅读设置。"
        )
        GuideItem(
            JerreaderIcon.SPEECH_BUBBLE,
            "悬浮译文",
            "译文框会与完整选区留出安全距离，框内则按实际译文收紧，不会为了段落模式强行留出大块空白；长译文达到高度上限后可在框内上下滑动。竖排时优先放在原文左侧或右侧；也可拖动顶部手柄手动调整。"
        )
        GuideItem(
            JerreaderIcon.DICTIONARY,
            "查词与生词本",
            "“学习”栏包含翻译、生词本和历史三部分；词典查询合并在翻译的“词语”模式里，译出一个词的同时会补充词典形、读音、词性和释义。阅读时长按一个词，从菜单选“查词”也可直接加入生词本。"
        )
        GuideItem(
            JerreaderIcon.SWAP,
            "独立翻译",
            "“学习 → 翻译”提供词语和句子两种模式。原文可自动识别或手动选择中文、英语、日语，译文也可在三种语言间选择；页面顶部可单独选择本机翻译、AI API 或 AI 代理，不会改动阅读器的翻译设置。"
        )
        GuideItem(
            JerreaderIcon.BOOKS,
            "日语学习增强",
            "日语词条会显示词典形、假名、词性、常见活用和例句；配置 AI 后可结合书中原句继续分析词语的语法角色与语境义。"
        )
        GuideItem(
            JerreaderIcon.SEARCH,
            "AI 句子结构分析",
            "译文卡片更多菜单里的“AI 句子结构分析”可分析句子主干、成分、从句或修饰关系，" +
                "以及英语时态、日语助词和活用等语法点；需要先配置 AI API 或代理。"
        )
        GuideItem(
            JerreaderIcon.SHARE,
            "学习内容导出",
            "在“学习”页选择 CSV、Markdown 或 Anki TSV，分享导出到其他应用。"
        )
        GuideItem(
            JerreaderIcon.TREND,
            "书籍管理与统计",
            "书架显示累计阅读时长和进度；每本书的更多菜单可更换封面，修改书名、作者、语言、系列、文件夹和标签，顶部批量整理可一次处理多本书。"
        )
        GuideItem(
            JerreaderIcon.SHIELD,
            "离线能力",
            "阅读、书架、已缓存的翻译与查词结果可离线使用；默认的本机翻译在语言模型下载完成后也可离线运行。"
        )
    }
}
...
private fun JapaneseSection() {
    GuideSection("日文版式") {
        GuideItem(
            JerreaderIcon.BOOKS,
            "原书",
            "保留出版物声明的横排、竖排和翻页方向；传统日文竖排书通常从右向左翻页。"
        )
        GuideItem(
            JerreaderIcon.TEXT_FORMAT,
            "横排",
            "把可重排日文 EPUB 统一改为横排，并让章节间和章节内都从左向右翻页。"
        )
        GuideFootnote(
            "阅读器的“文字方向”与“设置 → 默认阅读排版”都只提供这两项：竖排书在“原书”下本来就竖排，" +
                "对不是竖排排版的书强制竖排没有意义。可在设置里决定新书的初始偏好，也可在某本书的阅读设置中单独覆盖。"
        )
    }
}
...
private fun BackupSection() {
    GuideSection("备份与恢复") {
        ...
        GuideItem(
            JerreaderIcon.SHIELD,
            "手动备份",
            "在“手动备份”里勾选“书籍与封面”“进度、书签与批注”“生词与翻译收藏”“阅读与翻译偏好”中需要的范围，" +
                "再点“立即备份到当前文件夹”。包含书籍文件的备份明显更大，但重装后无需重新找书。"
        )
        GuideItem(
            JerreaderIcon.CLOCK,
            "自动备份",
            "开启“定期自动备份”后，按所选的备份间隔在打开书架时检查是否到期，只写入当前文件夹；" +
                "自动备份的范围可与手动备份分开勾选。"
        )
        GuideItem(
            JerreaderIcon.TRASH,
            "保留策略",
            "可分别设置保存时间、最多份数和总容量上限。超出限制时从最旧的一份开始删除，但永远保留最新的一份。"
        )
        GuideItem(
            JerreaderIcon.SWAP,
            "恢复与合并",
            "重装后重新选择原来的文件夹，备份会出现在列表中，点右侧“恢复”即可；也可用“从备份文件恢复 → 选择备份文件”从其他位置恢复。恢复只做合并，已存在的书籍与记录不会产生重复。"
        )
        GuideItem(
            JerreaderIcon.LOCK,
            "不进入备份的内容",
...
private fun FormatSection() {
    GuideSection("格式说明") {
        GuideLabeledRow("EPUB", "完整阅读与翻译")
        GuideLabeledRow("PDF", "点句翻译＋本机文字识别")
        GuideLabeledRow("DOCX", "转换正文后阅读")
        GuideLabeledRow("TXT", "自动识别常用编码")
...
/**
 * One guide entry, laid out the way iOS lays out its `Label`: a tinted glyph in
 * the leading column, title and detail stacked beside it.
 *
 * The icon is not decoration — it is the alignment grid. Without it the Android
 * guide was a wall of undifferentiated bold-then-grey paragraphs while the same
 * content on iOS scanned as a list, which is the format difference this screen
 * was reported for.
 */
@Composable
private fun GuideItem(icon: JerreaderIcon, title: String, detail: String) {
    Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
        JerreaderGlyph(
            icon = icon,
...
/**
 * A two-column row for the format table, mirroring iOS's `LabeledContent`.
 * Those four rows are a table on iOS and were title/detail paragraphs here.
 */
@Composable
private fun GuideLabeledRow(label: String, value: String) {
    Row(
        modifier = Modifier.fillMaxWidth(),
