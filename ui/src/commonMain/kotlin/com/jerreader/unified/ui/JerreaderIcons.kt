...
fun JerreaderGlyph(
    ...
    Canvas(modifier = modifier) {
        ...
        when (icon) {
            ...
            JerreaderIcon.FOLDER -> drawFolder(tint, unit)
...
/** Filled folder, matching iOS's `folder.fill` on the shelf's folder cards. */
private fun DrawScope.drawFolder(tint: Color, unit: Float) {
    val c = centered(unit)
    val left = c.x - unit * 0.30f
    val right = c.x + unit * 0.30f
    val top = c.y - unit * 0.22f
    val bottom = c.y + unit * 0.24f
    // The tab is a shorter rounded rect peeking above the body; drawing it
    // first lets the body's corner radius cover the seam between them.
    drawRoundRect(
        color = tint,
        topLeft = Offset(left, top - unit * 0.09f),
        size = androidx.compose.ui.geometry.Size(unit * 0.28f, unit * 0.18f),
        cornerRadius = androidx.compose.ui.geometry.CornerRadius(unit * 0.05f)
    )
    drawRoundRect(
        color = tint,
        topLeft = Offset(left, top),
