import AVFoundation
import CoreGraphics
// Scoped: JerreaderCore also exports `TranslationService`, `LanguageCode` and
// friends, which collide with the app's Swift protocols of the same name.
import class JerreaderCore.ReaderSelectionHighlightMetrics
import PDFKit
...
final class ReaderServicesTests: XCTestCase {
    ...
    func testStandaloneInputDoesNotOverwriteActiveIMECandidate() {
        XCTAssertFalse(
            StandaloneTranslationInputSynchronization.shouldApplyModelText(
                modelText: "今天去学校",
                editorText: "今天去xuexiao",
                hasMarkedText: true
            )
        )
        XCTAssertFalse(
            StandaloneTranslationInputSynchronization.shouldApplyModelText(
                modelText: "日本語を入力",
                editorText: "日本語をにゅうりょく",
                hasMarkedText: true
            )
        )
    }

    func testStandaloneInputAppliesExternalChangesAfterIMECommit() {
        XCTAssertTrue(
            StandaloneTranslationInputSynchronization.shouldApplyModelText(
                modelText: "完整保留输入内容",
                editorText: "完整保留输入",
    ...
    /// The custom selection colour has been reported wrong more than once, and
    /// every time it was the same thing underneath: WebKit paints its own
    /// system-blue selection below our overlay, so the colour on screen was the
    /// user's colour over blue, and plain blue for as long as the overlay had
    /// nothing to paint. Suppressing the stock paint is what makes the setting
    /// mean what it says, and it has to happen for every book, not just the
    /// Japanese ones.
    func testSelectionPolicySuppressesTheStockSelectionPaintForEveryBook() {
        for detach in [true, false] {
            let script = EPUBReaderViewController.selectionPolicyScript(
                detachRubyAnnotations: detach
            )
            XCTAssertTrue(script.contains("'::selection {'"))
            XCTAssertTrue(script.contains("'::-moz-selection {'"))
            XCTAssertTrue(script.contains("'background: transparent !important;'"))
            XCTAssertTrue(script.contains("'color: inherit !important;'"))
        }
    }

    /// Both the tiles the page paints and the ones UIKit paints over a PDF read
    /// their geometry from the same shared metrics, so a selection cannot look
    /// like one thing in an EPUB and another in a PDF.
    func testSelectionTilesShareTheirGeometryAcrossEveryScene() {
        let metrics = ReaderSelectionHighlightMetrics.shared

        XCTAssertEqual(
            EPUBReaderViewController.selectionStrokeWidthCSS,
            String(format: "%g", locale: Locale(identifier: "en_US_POSIX"), metrics.STROKE_WIDTH)
        )
        XCTAssertTrue(
            EPUBReaderViewController.cornerRadiusExpression("rect.height")
                .contains("\(metrics.MAXIMUM_CORNER_RADIUS)")
        )
        XCTAssertTrue(
            EPUBReaderViewController.cornerRadiusExpression("rect.height")
                .contains("rect.height")
        )

        // A tile shorter than the ratio's turning point rounds by its own
        // height; a tall one is capped, so a heading is not a lozenge.
        XCTAssertEqual(metrics.cornerRadius(tileHeight: 10), 1.6, accuracy: 0.0001)
        XCTAssertEqual(metrics.cornerRadius(tileHeight: 80), metrics.MAXIMUM_CORNER_RADIUS)
        XCTAssertEqual(metrics.cornerRadius(tileHeight: 0), 0)
        XCTAssertEqual(metrics.cornerRadius(tileHeight: -5), 0)
    }
    ...
    func testQuickSentenceSpeechHighlightTracksDOMRangeAndExcludesRuby() async throws {
        ...
        let baseHighlight = try await EPUBReaderViewController.paintQuickSentenceHighlight(
            in: webView,
            at: point,
    ...
    func testQuickSentenceHighlightLandsOnTheTextInVerticalWriting() async throws {
        for writingMode in ["horizontal-tb", "vertical-rl"] {
            ...
            _ = try await EPUBReaderViewController.paintQuickSentenceHighlight(
                in: webView,
                at: point,
                utf16Range: 0 ..< 8,
                requestToken: "test-\(writingMode)",
                vertical: writingMode.contains("vertical")
            )
    ...
    /// A ruby-annotated sentence must highlight as one continuous shape.
    ///
    /// `getClientRects()` returns one box per text node plus one per ruby
    /// annotation, so 翔太 with its readings arrived as separate boxes with
    /// visible gaps between them: the highlight looked chopped up on exactly
    /// the Japanese books this reader exists for, while Android — which has
    /// always merged through `ReaderSelectionRectMerger` — drew one shape. The
    /// merge now happens in Swift between measuring and painting, through that
    /// same shared merger.
    func testQuickSentenceHighlightIsContinuousAcrossRubyAnnotations() async throws {
        let webView = WKWebView(frame: CGRect(x: 0, y: 0, width: 390, height: 640))
        webView.loadHTMLString(
            """
            <!doctype html><html lang="ja"><head>
            <meta name="viewport" content="width=device-width">
            <style>body { margin: 16px; font-size: 20px; line-height: 2; }</style>
            </head><body>
            <p id="p">あばらやに行こう、といいだしたのは\
            <ruby>翔<rt>しょう</rt></ruby><ruby>太<rt>た</rt></ruby>だった。</p>
            </body></html>
            """,
            baseURL: nil
        )
        ...
        // The whole sentence, ruby readings excluded — that is the text the
        // segmenter works on and the offsets the highlight is given.
        let baseText = "あばらやに行こう、といいだしたのは翔太だった。"
        let painted = try await EPUBReaderViewController.paintQuickSentenceHighlight(
            in: webView,
            at: point,
            utf16Range: 0 ..< (baseText as NSString).length,
            requestToken: "ruby-continuity",
            vertical: false
        )
        XCTAssertNotNil(painted)

        // Markers sharing a line must touch. A gap between two of them is the
        // break the reader sees around a ruby word.
        let gapsRaw = try await webView.evaluateJavaScript(
            """
            (() => {
              const overlay = document.getElementById(
                'jerreader-reader-quick-sentence-highlight'
              ...
              if (!overlay) return null;
              const markers = Array.from(overlay.children)
                .map((node) => node.getBoundingClientRect())
                .filter((rect) => rect.width > 0.5 && rect.height > 0.5);
              if (!markers.length) return null;
              const rows = [];
              markers.forEach((rect) => {
                const row = rows.find((candidate) => {
                  const overlap = Math.min(candidate.bottom, rect.bottom) -
                    Math.max(candidate.top, rect.top);
                  const shorter = Math.min(
                    candidate.bottom - candidate.top, rect.height
                  );
                ...
                if (row) {
                  ...
                  rows.push({ top: rect.top, bottom: rect.bottom, rects: [rect] });
                }
              });
              let gaps = 0;
              rows.forEach((row) => {
                ...
                for (let index = 1; index < sorted.length; index += 1) {
                  if (sorted[index].left > sorted[index - 1].right + 1.5) gaps += 1;
                }
              });
              return { markers: markers.length, rows: rows.length, gaps: gaps };
            })()
            """
        )
        let gaps = try XCTUnwrap(gapsRaw as? [String: Any], "No overlay was produced")
        XCTAssertGreaterThan((gaps["markers"] as? NSNumber)?.intValue ?? 0, 0)
        XCTAssertEqual(
            (gaps["gaps"] as? NSNumber)?.intValue,
            0,
            "The highlight broke apart around a ruby annotation"
        )
    }

    /// The selection colour must survive every reader theme.
    ///
    /// ReadiumCSS turns one of these on for every theme except the light one:
    ///
    ...
    ///       background-color: transparent !important; }
    ///
    /// A highlight tile is a `<span>` inside `<body>`, so all three match it and
    /// erase its fill. That — not the palette maths, which was rewritten several
    /// times — is why the custom selection colour worked on the light theme and
    /// looked transparent on sepia, cool grey, dark and every custom background.
    func testQuickSentenceHighlightKeepsItsFillUnderEveryReadiumThemeRule() async throws {
        let themeRules = [
            "night": ":root[data-jr] :not(a){color:inherit!important;"
                + "background-color:transparent!important;"
                + "border-color:currentColor!important}",
            "sepia": ":root[data-jr] :not(a){color:inherit!important;"
                + "background-color:transparent!important}",
            "custom": ":root[data-jr] *{background-color:transparent!important}"
        ]

        for (name, rule) in themeRules {
            let webView = WKWebView(frame: CGRect(x: 0, y: 0, width: 390, height: 640))
            webView.loadHTMLString(
                """
                <!doctype html><html lang="ja" data-jr="on"><head>
                <meta name="viewport" content="width=device-width">
                <style>body { margin: 16px; font-size: 20px; } \(rule)</style>
                </head><body>
                <p id="p">吾輩は猫である。名前はまだ無い。</p>
                </body></html>
                """,
                baseURL: nil
            )
            ...
            _ = try await EPUBReaderViewController.paintQuickSentenceHighlight(
                ...
                at: point,
                utf16Range: 0 ..< 8,
                requestToken: "theme-\(name)",
                vertical: false
            )

            let paintRaw = try await webView.evaluateJavaScript(
                """
                (() => {
                  const overlay = document.getElementById(
                    'jerreader-reader-quick-sentence-highlight'
                  );
                  const marker = overlay?.firstElementChild;
            ...
            let paint = try XCTUnwrap(
                paintRaw as? [String: Any],
                "No highlight marker was painted for \(name)"
            )
            let background = paint["background"] as? String ?? ""
            XCTAssertFalse(
                background.isEmpty || background.contains("rgba(0, 0, 0, 0)"),
                "\(name) erased the highlight fill — got \(background)"
            )
            XCTAssertFalse(
                (paint["borderColor"] as? String ?? "").isEmpty,
                "\(name) erased the highlight outline"
            )
        }
    }

    /// The highlight has to stay on its sentence while the page moves.
    ///
    /// A vertical book is always in scroll mode — ReadiumCSS refuses to
    /// paginate vertical text — so the spread is scrolled constantly, and
    /// Readium reports a location change for every one of those movements. The
    /// host used to tear the highlight down on each report, which is what left
    /// a translation card standing over a sentence with no colour on it. It now
    /// keeps the highlight for as long as the reader is in the same resource,
    /// which is only correct if the overlay really does travel with its text.
    func testQuickSentenceHighlightStaysOnItsSentenceAfterScrolling() async throws {
        let webView = WKWebView(frame: CGRect(x: 0, y: 0, width: 390, height: 640))
        let paragraph = String(
            repeating: "彼は扉を開けた。廊下には誰もいなかった。遠くで誰かが呼んでいる。",
            count: 40
        )
        webView.loadHTMLString(
            """
            ...
            <style>
              ...
              body { margin: 24px; font-size: 22px; line-height: 1.8; }
            </style>
        ...
        for _ in 0 ..< 200 {
            if try await webView.evaluateJavaScript(
                "document.getElementById('p') !== null && document.body.scrollWidth > 800"
            ) as? Bool == true {
                break
            }
            try await Task.sleep(for: .milliseconds(10))
        ...
        _ = try await EPUBReaderViewController.paintQuickSentenceHighlight(
            ...
            at: point,
            utf16Range: 0 ..< 8,
            requestToken: "scrolled-highlight",
            vertical: false
        )

        /// Where the markers sit relative to the first eight characters they
        /// were painted over — the only thing that has to stay constant.
        func offsetFromItsText() async throws -> CGPoint {
            let raw = try await webView.evaluateJavaScript(
                ...
                (() => {
                  ...
                  const left = Math.min(...markers.map((rect) => rect.left));
                  const top = Math.min(...markers.map((rect) => rect.top));
                  return { dx: left - target.left, dy: top - target.top };
                })()
                """
            )
            let json = try XCTUnwrap(raw as? [String: Any], "the overlay lost its markers")
            return CGPoint(
                x: CGFloat(truncating: try XCTUnwrap(json["dx"] as? NSNumber)),
                y: CGFloat(truncating: try XCTUnwrap(json["dy"] as? NSNumber))
            )
        }

        let before = try await offsetFromItsText()

        // Three viewports along the spread, which in `vertical-rl` is where the
        // reader goes next.
        webView.scrollView.contentOffset.x = max(
            webView.scrollView.contentOffset.x - 390 * 3,
            0
        )
        try await Task.sleep(for: .milliseconds(120))

        let after = try await offsetFromItsText()
        XCTAssertEqual(
            Double(before.x),
            Double(after.x),
            accuracy: 1.0,
            "the highlight drifted off its sentence when the page scrolled"
        )
        XCTAssertEqual(Double(before.y), Double(after.y), accuracy: 1.0)
    }
