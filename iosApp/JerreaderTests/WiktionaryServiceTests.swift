...
final class WiktionaryServiceTests: XCTestCase {
    ...
    func testDefaultServiceFallsBackToBundledJMdict() async throws {
        let service = DefaultLexicalLookupService(
            remote: AlwaysFailingLexicalLookupService()
        )
        ...
        XCTAssertEqual(result.reading, "ほん")
        XCTAssertTrue(result.definitions.contains { $0.localizedCaseInsensitiveContains("book") })
        XCTAssertEqual(result.sentenceContext, "本を読みます。")
        XCTAssertTrue(result.usageNote?.contains("离线 JMdict") == true)
    }
...
private struct AlwaysFailingLexicalLookupService: LexicalLookupService {
    func lookup(
        word: String,
