...
final class WordLookupStoreTests: XCTestCase {
    ...
    func testContextlessLookupPreservesLastSourceAndContextHistory() throws {
        let container = try makeContainer()
        let context = container.mainContext
        ...
        XCTAssertEqual(updated.sentenceContext, "She went home.")
        XCTAssertEqual(updated.sourceBookID, sourceBookID)
        XCTAssertEqual(updated.sourceBookTitle, "Example Book")
        XCTAssertEqual(updated.contextHistory, ["She went home."])
    }

    func testLearningStatusAndFiveNewestContextsPersist() throws {
        let container = try makeContainer()
        let context = container.mainContext
        var record: WordLookupRecord?
        for index in 1 ... 7 {
            record = try WordLookupStore.record(
                englishExplanation(
                    surfaceForm: "went",
                    lemma: "go",
                    sentenceContext: "Context \(index)"
                ),
                in: context
            )
        }
        let saved = try XCTUnwrap(record)
        try WordLookupStore.setFavorite(true, for: saved, in: context)
        XCTAssertEqual(saved.vocabularyStatusRawValue, "learning")
        XCTAssertEqual(saved.contextHistory.count, 5)
        XCTAssertEqual(saved.contextHistory.first, "Context 7")
        XCTAssertEqual(saved.contextHistory.last, "Context 3")

        let known = VocabularyStatus.companion.fromStorageId(id: "known")
        try WordLookupStore.setVocabularyStatus(known, for: saved, in: context)
        XCTAssertEqual(saved.vocabularyStatusRawValue, "known")
    }
    ...
    func testReviewPersistsScheduleAndForgottenCardReturnsToLearning() throws {
        let container = try makeContainer()
        let context = container.mainContext
        let record = try WordLookupStore.record(
            WordExplanation(
                ...
                inflectionNote: "礼貌体过去式",
                examples: [
                    WordExample(sourceText: "寿司を食べました。", translatedText: "吃了寿司。"),
                ],
                usageNote: nil,
                sentenceContext: "昨日、寿司を食べました。"
            ),
            in: context
        )
        try WordLookupStore.setFavorite(true, for: record, in: context)
        ...
        XCTAssertEqual(record.reviewIntervalDays, 1)
        XCTAssertEqual(record.lastReviewedAt, firstDate)
        XCTAssertEqual(
            record.nextReviewAt,
            firstDate.addingTimeInterval(24 * 60 * 60)
        )
        XCTAssertEqual(record.vocabularyStatusRawValue, "learning")
        ...
        XCTAssertEqual(record.reviewLapseCount, 1)
        XCTAssertEqual(record.vocabularyStatusRawValue, "learning")
        XCTAssertEqual(
            record.nextReviewAt,
            firstDate.addingTimeInterval(11 * 60)
        )
    }
