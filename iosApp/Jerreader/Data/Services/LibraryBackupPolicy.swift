...
struct LibraryBackupVault: @unchecked Sendable {
    ...
    /// The folder backups are actually written to — the user-selected one when
    /// there is one, otherwise the app's own `Documents/Jerreader Backups`.
    /// The file picker needs this: without a `directoryURL` it opens at
    /// Recents, where local backups that the settings list is happily showing
    /// are nowhere to be found.
    func currentDirectoryAccess() throws -> LibraryBackupDirectoryAccess {
        try directoryAccess(create: true)
    }
    ...
    private func listBackups(
        ...
        for url in urls where LibraryBackupService.isBackupArchive(url) {
    ...
    func delete(_ backup: LibraryBackupArchiveInfo) throws {
        ...
        guard target.deletingLastPathComponent() == directory,
              LibraryBackupService.isBackupArchive(target)
        else {
