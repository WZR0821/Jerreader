...
struct LibraryBackupService: @unchecked Sendable {
    /// What a new archive is called after the timestamp.
    ///
    /// It ends in `.zip`, and that is the whole point. The archive has always
    /// been a ZIP, but naming it `.jerreader-backup` meant every File Provider
    /// had to resolve a custom extension to the app's exported UTI before the
    /// document picker would let the file be chosen — and iCloud Drive
    /// routinely does not, especially for an item it has not materialized yet.
    /// The backup then sits there in Files, visible and greyed out, which is
    /// 「备份还是不能手动选择文件」 reported twice. Widening the picker's accepted
    /// types did not fix it. A `.zip` needs nothing resolved: it is selectable
    /// in every picker, and it can be mailed, AirDropped or copied to a
    /// computer like any other file. `jerbackup` stays in the middle of the
    /// name so the file still says what it is at a glance, and so it matches
    /// what Android has been writing all along — the two apps restore each
    /// other's archives, so they should not disagree about the name.
    ///
    /// Carried without its leading dot, because every call site builds the
    /// name with the separator already in place.
    static let fileNameExtension = String(
        LibraryBackupNaming.shared.ARCHIVE_SUFFIX.dropFirst()
    )
    static let typeIdentifier = "com.jerreader.backup"

    /// Whether [url] names a Jerreader backup, in either naming scheme.
    ///
    /// Only a name test — every caller still validates the manifest, so a
    /// stray `.zip` sitting in the backup folder costs a failed inspection and
    /// nothing else.
    static func isBackupArchive(_ url: URL) -> Bool {
        LibraryBackupNaming.shared.isArchiveName(name: url.lastPathComponent)
    }
