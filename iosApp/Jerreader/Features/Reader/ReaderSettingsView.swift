    ...
    private var backgroundSection: some View {
        Section {
                ...
                ForEach(ReaderThemeChoice.allCases) { theme in
                    Button {
                        withAnimation(reduceMotion ? nil : JerreaderMotion.stateChange) {
                            // A built-in background drops the whole custom
                            // pair. Keeping the custom selection colour would
                            // leave a hand-picked highlight on a page it was
                            // never chosen against.
                            model.updateCustomBackgroundHex("")
                            model.updateCustomSelectionColorHex("")
            ...
            ReaderColorPresetSection(
                theme: model.theme,
                backgroundHex: model.customBackgroundHex,
                selectionHex: model.customSelectionColorHex
            ) { backgroundHex, selectionHex in
                withAnimation(reduceMotion ? nil : JerreaderMotion.stateChange) {
                    model.updateCustomBackgroundHex(backgroundHex)
                    model.updateCustomSelectionColorHex(selectionHex)
                }
            }
        } header: {
            Text("背景")
