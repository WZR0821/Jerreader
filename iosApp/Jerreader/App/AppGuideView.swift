...
struct AppGuideView: View {
    ...
    private var readingSection: some View {
        Section("如何阅读与翻译") {
            GuideItem(
                title: "翻页",
                detail: "轻点页面左侧或右侧快速翻页，也可使用底部的大号翻页按钮。轻点正文的空白处（没有文字的地方）可以收起或重新显示上下两条控制栏。",
                systemImage: "rectangle.portrait.on.rectangle.portrait"
            )
            GuideItem(
                title: "点句或点段翻译",
                detail: "工具栏中的“字”图标为开启状态时，可在阅读设置的“翻译范围”选择句子或段落，再轻点正文翻译；开启翻页保护时，滑动仍可翻页，点按不会同时触发翻页。轻点没有文字的位置不会触发翻译，只会收起或显示控制栏。",
    ...
    private var featureSection: some View {
        Section("功能介绍") {
            GuideItem(title: "界面主题", detail: "设置 → 界面主题可选择海蓝、森林、紫藤、琥珀或莓红色系；按钮、卡片和背景会一起变化，并自动适配系统浅色或深色外观。", systemImage: "paintpalette.fill")
            GuideItem(title: "阅读设置与选区色", detail: "每本书可独立保存字体、字号、背景、页边距、行距、段距、翻页模式和选区颜色。浅色、护眼、冷灰、深色会自动采用适配的选区色；自定义背景可另选选区色，并把这一组配色存成自己的背景套系。设置 → 默认阅读排版可决定新书样式，并用开关持续同步或立即应用到现有全部书籍。", systemImage: "textformat.size")
    ...
    private var japaneseSection: some View {
        Section("日文版式") {
            ...
            Text("这里只提供“原版”和“横排”两项：竖排书在“原版”下本来就竖排，对不是竖排排版的书强制竖排没有意义。可在“设置 → 默认阅读排版”选择新书初始偏好，也可在某本书的阅读设置中单独覆盖。即使 EPUB 没写语言标签，Jerreader也会检查书内 CSS 来恢复原版竖排。")
                .font(.caption)
                .foregroundStyle(.secondary)
        }
    }
