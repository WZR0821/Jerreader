...
object ReaderSentenceSegmenter {
    ...
    /**
     * Every sentence in [text], in order, using the same boundary rules as
     * [sentenceRange]. Whitespace between sentences is dropped; whitespace
     * inside one is left alone.
     */
    fun sentences(text: String, language: LanguageCode?): List<String> {
        ...
        while (index < text.length) {
            if (!isBoundaryAfter(text, index, japanese, quotes)) {
                index++
                continue
            }
            var end = index + 1
            while (end < text.length && text[end] in CLOSING_MARKS) end++
            text.substring(start, end).trim().takeIf { it.isNotEmpty() }?.let { pieces += it }
            start = end
            index = end
        }
        text.substring(start).trim().takeIf { it.isNotEmpty() }?.let { pieces += it }
        return pieces
    }
